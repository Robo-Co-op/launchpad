...
### Step 1: 名前の確認
`.user_name` ファイルが存在すれば読み取り、「Hi [名前]! Welcome to Launchpad!」と挨拶する。
ファイルがなければ「How should I call you?」と聞く。
...
- 「起業したい」「始めたい」「ビジネスしたい」「Let's start」等と言ったら、Step 2から始める
...
### 初回（オンボーディング）
```
1. 名前を確認（.user_nameファイル or 「How should I call you?」）
